...
<template>
    <section class="bg-white shadow-xl rounded-xl overflow-hidden">
        <div :class="`w-full h-[200px] p-5 bg-gray-300`">
            <img :src="props.product.image" class="w-full h-full object-contain h-auto w-auto"/>
        </div>
        <div class="px-5 pb-5 pt-9 relative">
            ...
            <div :class="`cursor-pointer absolute -top-5 right-7 w-[50px] h-[50px] bg-red-600 
            shadow-xl rounded-full flex justify-center items-center hover:bg-white
            hover:text-red-600 font-bold transition duration-300`">
            <i class="ri-shopping-cart-2-line">Hot!</i>
            </div>
        </div>
    </section>
</template>